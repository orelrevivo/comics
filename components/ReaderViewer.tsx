...
import { updateReaderSettingsAction } from '@/app/actions/auth';
...
export default function ReaderViewer({
  ...
  const [settings, setSettings] = useState<ReaderSettings>(initialSettings || defaultSettings);
  ...
  useEffect(() => {
    if (story?.id && chapter?.id) {
      localStorage.setItem(`last_read_${story.id}`, chapter.id);
    }
  }, [story?.id, chapter?.id]);
  ...
  useEffect(() => {
    ...
    // Restore sticky state when leaving the reader
    return () => {
      if (navbar) {
        navbar.classList.add('sticky', 'top-0');
      }
    };
  }, [settings.stickyTopNav]);
  ...
  const updateSetting = <K extends keyof ReaderSettings>(key: K, value: ReaderSettings[K]) => {
    ...
    if (isLoggedIn) {
      updateReaderSettingsAction(JSON.stringify(newSettings)).catch(console.error);
    }
  };
    ...
    <>
      {!settings.darkBackground && (
        <style dangerouslySetInnerHTML={{
          __html: `
          :root { --background: #ffffff !important; --foreground: #171717 !important; }
          body { background-color: #ffffff !important; color: #18181b !important; }
          .dark\\:bg-black { background-color: #ffffff !important; }
          .dark\\:bg-zinc-950 { background-color: #ffffff !important; }
          .dark\\:bg-zinc-950\\/95 { background-color: rgba(255,255,255,0.95) !important; }
          .dark\\:bg-zinc-900 { background-color: #ffffff !important; }
          .dark\\:bg-zinc-800 { background-color: #f4f4f5 !important; }
          .dark\\:border-zinc-800 { border-color: #e4e4e7 !important; }
          .dark\\:border-zinc-800\\/50 { border-color: rgba(228,228,231,0.5) !important; }
          .dark\\:text-zinc-50 { color: #18181b !important; }
          .dark\\:text-zinc-100 { color: #18181b !important; }
          .dark\\:text-zinc-200 { color: #27272a !important; }
          .dark\\:text-zinc-300 { color: #3f3f46 !important; }
          
          /* Brutal UI Overrides */
          .dark .brutal-btn, .dark .brutal-card, .dark .brutal-input {
      ...
      <div className={`min-h-screen pb-20 ${bgClass} transition-colors duration-200`}>
        {/* Top Navigation Bar */}
        <div className={`${settings.stickyTopNav ? 'sticky top-[152px]' : ''} z-40 w-full border-b backdrop-blur-md ${navBgClass}`}>
          <div className="container mx-auto flex h-14 items-center justify-between px-4">
            <div className="flex items-center gap-4">
              ...
              <div className="font-semibold">
                <span className="font-normal opacity-70 ml-2 hidden sm:inline">{chapter.title}</span>
              </div>
            </div>
