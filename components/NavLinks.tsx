...
export default function NavLinks() {
  ...
  const getLinkClass = (isActive: boolean) => {
    return `relative whitespace-nowrap py-1 transition-colors hover:text-indigo-600 dark:hover:text-indigo-400 after:absolute after:inset-x-0 after:-bottom-[1px] after:h-[2px] after:scale-x-0 after:bg-indigo-600 after:transition-transform after:duration-200 after:content-[''] hover:after:scale-x-100 dark:after:bg-indigo-400 ${
      isActive ? 'text-indigo-600 after:scale-x-100 dark:text-indigo-400' : ''
    }`.trim();
  };

  return (
    <nav className="flex items-center gap-6 text-sm font-medium text-zinc-600 dark:text-zinc-400 overflow-x-auto z-50">
      <Link
        href="/"
